 ...
 * @author Ratul
 */
public class AdminMenuItem extends JPanel {
    ...
    private final List<AdminMenuEvent> adminevents;
    ...
    private AdminPopupSubmenu adminpopup;

    public AdminMenuItem(AdminMenu adminmenu, String menus[], int menuIndex, List<AdminMenuEvent> events) {
    ...
    private Icon getIcon() {
        ...
        FlatSVGIcon icon = new FlatSVGIcon("adminMenu/icon/" + menuIndex + ".svg");
        ...
        for (int i = 0; i < menus.length; i++) {
            ...
            if (i == 0) {
                ...
                menuItem.addActionListener((ActionEvent e) -> {
                    if (menus.length > 1) {
                        if (adminmenu.isMenuFull()) {
                            AdminMenuAnimation.animate(AdminMenuItem.this, !menuShow);
                        } else {
                            adminpopup.show(AdminMenuItem.this, (int) AdminMenuItem.this.getWidth() + UIScale.scale(5), UIScale.scale(menuItemHeight) / 2);
                        }
                    } else {
                        adminmenu.runEvent(menuIndex, 0);
                    }
                });
            } else {
                final int subIndex = i;
                menuItem.addActionListener((ActionEvent e) -> {
                    adminmenu.runEvent(menuIndex, subIndex);
                });
            }
            add(menuItem);
        }
        adminpopup = new AdminPopupSubmenu(getComponentOrientation(), adminmenu, menuIndex, menus);
    }
    ...
    protected void setSelectedIndex(int index) {
        ...
        adminpopup.setSelectedIndex(index);
    }
